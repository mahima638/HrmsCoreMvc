...
namespace HrmsCoreMvc.Services.Reports
{
    public class AllProjectsReportService : IAllProjectsService
    {
        ...
        {
            var projects = await db.AllProjects
            ...
            int CompletedTasks = projects
                .Where(x => x.Tasks != null)
                .SelectMany(x => x.Tasks)
                .Count(t => t.Status == "Completed");

            int InprogressTasks = projects
                .Where(x => x.Tasks != null)
                .SelectMany(x => x.Tasks)
                .Count(t => t.Status == "Inprogress");
            ...
            return new ProjectChartDto
            {
                ...
                ActiveProjects = new List<double>
                {
                    total > 0 ? Math.Round(((double)ActiveProjects / total) * 100, 2) : 0
                },

                InActiveProjects = new List<double>
                {
                    total > 0 ? Math.Round(((double)InActiveProjects / total) * 100, 2) : 0
                },

                InProgressTasks = new List<double>
                {
                    total > 0 ? Math.Round(((double)InprogressTasks / total) * 100, 2) : 0
                },

                CompletedTasks = new List<double>
                {
                    total > 0 ? Math.Round(((double)CompletedTasks / total) * 100, 2) : 0
                }
            };
        }
        ...
        {
            var data = await db.AllProjects
                .Where(ap => ap.EndDate < DateTime.Now)
                .CountAsync();
        ...
        public async Task<IEnumerable<ProjectReportsViewModel>> sortProjectReports(
            string? priorityType,
            string? statusType,
            string? sortType)
        {
            ...
            {
                query = query.OrderByDescending(ap => ap.EndDate);
            }
